...
type Dictionary struct {
	...
	Tokens      []gse.Token // 词典中所有的分词，方便遍历
...
// AddToken 向词典中加入一个分词
func (dict *Dictionary) AddToken(token gse.Token) error {
	bytes := []byte(token.Text())
	...
	if len([]byte(token.Text())) > dict.maxTokenLen {
		dict.maxTokenLen = len([]byte(token.Text()))
	}
...
func (dict *Dictionary) RemoveToken(token gse.Token) error {
	bytes := []byte(token.Text())
...
func (dict *Dictionary) LookupTokens(
	words []gse.Text, tokens []*gse.Token) (numOfTokens int) {
	var (
		id, value int
		err       error
	)
